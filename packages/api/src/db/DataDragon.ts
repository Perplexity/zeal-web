...
type ChampionGetter = {
	all: (version: string) => Promise<unknown>;
	byName: (champName: string, version: string) => Promise<unknown>;
}
...
export class DDragon {
	...
	get champion(): ChampionGetter {
		return {
			all: async (version: string): Promise<unknown> => {
				const v = version || (await this.versions.latest());
				return await this.request(`/cdn/${v}/data/en_GB/champion.json`);
			},
			byName: async (champName: string, version: string): Promise<unknown> => {
				const v = version || (await this.versions.latest());
